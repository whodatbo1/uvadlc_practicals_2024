...
import math
from gpt import CausalSelfAttention, TransformerDecoderBlock  # Replace with the actual import path
...
class TestCausalSelfAttention(unittest.TestCase):
    ...
    def test_rms_norm(self):
        ...
        y_ours = RMSNorm(config.n_embd)(x)
        y_test = F.rms_norm(x, normalized_shape=[config.n_embd],weight=torch.ones(config.n_embd), eps=1e-6)
        print(f'y_ours: {y_ours}')
        print(f'y_test: {y_test}')
        self.assertEqual(y_ours.shape, x.shape)
        self.assertTrue(torch.allclose(y_ours, y_test, atol=1e-5))
    ...
    #     xq_rot_test = rotary_emb.rotate_queries_or_keys(xq)
    #     xk_rot_test = rotary_emb.rotate_queries_or_keys(xk)
    ...
    #     self.assertTrue(torch.allclose(xq_rot, xq_rot_test, atol=1e-5))
    #     self.assertTrue(torch.allclose(xk_rot, xk_rot_test, atol=1e-5))
    ...
    def test_decoder_layer(self):
        config = MockConfig()
        module = TransformerDecoderBlock(config)
        x = torch.randn(1, 10, config.n_embd)
        output = module(x)
